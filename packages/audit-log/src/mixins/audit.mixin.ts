...
import {AbstractConstructor, AuditMixinBase, AuditOptions, IAuditMixin, IAuditMixinOptions} from '../types';
...
>(superClass: R, opts: IAuditMixinOptions): R & AbstractConstructor<IAuditMixin<UserID>> {
  abstract class MixedRepository extends superClass implements IAuditMixin<UserID> {
    ...
    async create(dataObject: DataObject<M>, options?: AuditOptions): Promise<M> {
      ...
      if (this.getCurrentUser && !options?.noAudit) {
        ...
        auditRepo.create(audit).catch(() => {
          //sonarignore:start
          console.error(`Audit failed for data => ${JSON.stringify(audit.toJSON())}`);
          //sonarignore:end
        });
      }
      return created;
    ...
    async createAll(dataObjects: DataObject<M>[], options?: AuditOptions): Promise<M[]> {
      ...
      if (this.getCurrentUser && !options?.noAudit) {
        ...
        auditRepo.createAll(audits).catch(() => {
          ...
          //sonarignore:start
          console.error(`Audit failed for data => ${JSON.stringify(auditsJson)}`);
          //sonarignore:end
        });
      }
      return created;
    ...
    async updateAll(dataObject: DataObject<M>, where?: Where<M>, options?: AuditOptions): Promise<Count> {
      if (options?.noAudit) {
    ...
    async updateById(id: ID, data: DataObject<M>, options?: AuditOptions): Promise<void> {
      if (options?.noAudit) {
      ...
      if (this.getCurrentUser) {
        ...
        auditRepo.create(auditLog).catch(() => {
          //sonarignore:start
          console.error(`Audit failed for data => ${JSON.stringify(auditLog.toJSON())}`);
          //sonarignore:end
        });
      }
    }

    async replaceById(id: ID, data: DataObject<M>, options?: AuditOptions): Promise<void> {
      if (options?.noAudit) {

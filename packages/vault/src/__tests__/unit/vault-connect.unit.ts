...
describe('Vault Service', () => {
  describe('Vault Service', () => {
    ...
    it('returns error message when cannot initialize', async () => {
      const vaultConnector = new VaultConnector(config);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      jest.spyOn((vaultConnector as any)._vaultClient, 'init').mockImplementation(() => ({}));
      await expect(vaultConnector.read('dummyPath')).rejectedWith(/Vault initialisation failed !/);
    });
  });
});

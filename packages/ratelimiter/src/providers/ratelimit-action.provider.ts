...
export class RatelimitActionProvider implements Provider<RateLimitAction> {
  ...
  async action(request: Request, response: Response): Promise<void> {
    ...
    const promise = new Promise<void>((resolve, reject) => {
      ...
      const opts: Writable<RateLimitOptions> = Object.assign({}, this.config, operationMetadata);

      if (dataStore) {
        opts.store = dataStore;
      }

      opts.message = new HttpErrors.TooManyRequests(opts.message?.toString() ?? 'Method rate limit reached !');

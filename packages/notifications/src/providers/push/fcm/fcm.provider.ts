...
export class FcmProvider implements Provider<FcmNotification> {
  ...
  sendingPushToReceiverTokens(message: FcmMessage, generalMessageObj: GeneralMessage) {
  ...
  sendingPushToTopics(message: FcmMessage, generalMessageObj: GeneralMessage) {
    const promises: Promise<string | admin.messaging.BatchResponse>[] = [];
    const topics = message.receiver.to.filter(item => item.type === FcmSubscriberType.FCMTopic);
    ...
    if (topics.length > 0) {
      ...
      topics.forEach(topic => {
        ...
        promises.push(this.fcmService.messaging().send(msgToTransfer, (message.options.dryRun = false)));
      });
    }
  ...
  sendingPushToConditions(message: FcmMessage, generalMessageObj: GeneralMessage) {
    const promises: Promise<string | admin.messaging.BatchResponse>[] = [];
    const conditions = message.receiver.to.filter(item => item.type === FcmSubscriberType.FCMCondition);

...
export class CasbinAuthorizationProvider implements Provider<CasbinAuthorizeFn> {
  ...
  async action(user: IAuthUserWithPermissions, resource: string, request?: Request): Promise<boolean> {
    ...
    try {
      ...
      // In case casbin policy is coming via provider, use that to initialise enforcer
        ...
        await enforcer.initWithModelAndAdapter(casbinConfig.model as casbin.Model, stringAdapter);
      } else {
        return false;
      }

...
#### `new ObjectQuery(repository)`

通过 `new ObjectQuery(repository)` 构造，传入一个 Repository 实例, 可支持 include。

#### `new ObjectQuery(entityClass, repository)`

通过 `new ObjectQuery(entityClass, repository)` 构造，传入一个模型类和一个 Repository 实例, 不支持 include。

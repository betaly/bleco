...
export class KeycloakStrategyFactoryProvider implements Provider<KeycloakStrategyFactory> {
  ...
  getKeycloakAuthStrategyVerifier(
    ...
    const strategy = new KeycloakStrategy(
      options,
      async (accessToken: string, refreshToken: string, profile: Keycloak.Profile, cb: Keycloak.VerifyCallback) => {
        try {
    ...
    strategy.userProfile = (accessToken: string, done: (err: unknown, userInfo?: unknown) => void) => {
  ...
  private _userProfileFn(
    ...
    strategy._oauth2.get(strategy.options.userInfoURL, accessToken, (err: unknown, body: string) => {
      if (err) {
        return done(err);
      }
      ...
      try {
        ...
        for (const key of ['sub', 'name', 'given_name', 'family_name', 'middle_name', 'preferred_username']) {
          delete userInfo[key];
        }
